...
//                        p.$jot('.jt-content, .jt-title').css('width', 'auto');
    ...
    setSubproductQuantityConditions: function (tempField, condition) {
        if (tempField.match(/input_[0-9]+_quantity_[0-9]+_[0-9]+/)) {
            ...
            var pItem = document.querySelector('.form-product-item[pid="' + pid + '"]');
    ...
    checkCalculation: function (calc) {
        ...
        try {
            if (![
                ...
                'file', 'other', 'mixed', 'sofort', 'payoneer', 'paysafe', 'iyzico', 'gocardless', 'stripeACH', 'paypalSPB', 'cybersource', "paypalcomplete", 'inline', 'appointment',
            ...
            case "html":
                try {
                    ...
                    if (spans.length == 0) {
                      if (resultFieldType === 'inline') {
                        ...
                        fitbFormFields.forEach(function(el) {
                          if (el.innerHTML) {
                            el.innerHTML = el.innerHTML.replace(fieldRegex, '$1<span class="replaceTag ' + className + '" default="'+def+'"></span>$2');
                          }
                        });
                      } else {
                        var contents = calc.isLabel ? $('label_' + result).innerHTML : $('text_' + result).innerHTML;
                        if (calc.isLabel) {
                          contents = contents.replace(re, '<span class="replaceTag ' + className + '" default="'+def+'"></span>');
                        } else {
                    ...
                    subLabel.each(function (subl){
                        var content = subl.innerHTML.replace(re, '<span class="replaceTag ' + className + '" default="'+def+'"></span>');
                        subl.update(content);
                    });
                    ...
                    if (description) {
                        ...
                        if (descriptionContent.indexOf(calc.replaceText) > -1) {
                          descriptionContent = descriptionContent.replace(re, '<span class="replaceTag ' + className + '" default="'+def+'"></span>');
   ...
   */
    ...
    handlePages: function () {
        ...
        function validatePaymentOnNextBtn(paymentName) {
            var paymentInCurrentSection = JotForm.currentSection.down('li[data-payment="true"]');
            if (!window.paymentType || !paymentInCurrentSection) { return Promise.resolve(true); }

            var isProductList = paymentInCurrentSection.getAttribute('data-type') === 'control_payment';
            var isPaymentHidden = paymentInCurrentSection.classList.contains('form-field-hidden');

            if (isProductList || isPaymentHidden) {
                return Promise.resolve(true);
            }

            return ValidatePaymentGateways.validate(paymentName);
        }
        ...
        $$('.form-pagebreak').each(function (page, i) {
            ...
            section.select('.form-pagebreak-next').invoke('observe', 'click', function () { // When next button is clicked
                validatePaymentOnNextBtn(JotForm.payment).then(function(isPaymentValid) {
                    if (JotForm.saving || JotForm.loadingPendingSubmission) {
                        return;
                    }
                    if ((JotForm.validateAll(form, section) && isPaymentValid) || getQuerystring('qp') !== "") {
  ...
  */
  handleSignSignatureInputs: function() {
    ...
    validateAll: function (form, scopeSelector) {
        ...
        this.handleTextareaLimits(false);
            ...
            widgetInputs.each(function (el) {
                ...
                try {
                    if (el.hasClassName('widget-required') && !el.hasClassName('form-validation-error')) {
                        ...
                        var isWidgetHidden = el.up('.form-field-hidden') || el.up('.always-hidden');
                        ...
                        if ((parentSection && parentSection.style && parentSection.style.display !== 'none') && !isWidgetHidden && JotForm.isVisible(el)) {
            ...
            if (!(!!input.validateInput && input.validateInput())) {
                ...
                try {
                    ...
                    if (corInputId && allInputValidations.include('required') || (input.hasClassName('validate') && input.hasClassName('valid'))) {
                        var isInputHidden = input.up('.form-field-hidden') || input.up('.always-hidden');
                        if (isInputHidden && JotForm.validatedRequiredFieldIDs[corInputId]) {
                            delete JotForm.validatedRequiredFieldIDs[corInputId];
                        }
                        // don't check hidden fields or any other fields that are on the other pages.
                        if ((parentSection && parentSection.style && parentSection.style.display !== 'none') && !isInputHidden && JotForm.isVisible(input)) {
